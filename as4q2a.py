...
for q in q_range:
    ...
    Xpoly = PolynomialFeatures(q).fit_transform(X2)
    ...
    for C in C_range:
        ...
        for train, test in kf.split(Xpoly):
            ...
            if((q == 1) or (q == 2) or (q == 6)) and not plotted:
                plt.scatter(X2, y, color ='blue', label = 'training data')
                ypred = model.predict(Xpoly)
                plt.scatter(X2, ypred, color = 'orange', label = 'predictions')
                plt.xlabel("input X2"); plt.ylabel("output y")
                plt.title("2D plot")
                plt.legend()
